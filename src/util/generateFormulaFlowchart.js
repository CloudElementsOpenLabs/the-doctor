...
const {contains, append, find, propEq, pipe, filter, forEach, max, join, isNil, concat, toLower, T, reduce, equals, chain, toUpper, keys, length, cond, replace, forEachObjIndexed, map} = require('ramda');
...
const objectToPrintOut = object => {
    ...
    return join('<br>', map(k => `${align(k, maxChars, 'right')} : ${object[k].toString().replace('"',"'")}`)(keys(object)))
}
...
const getShape = pipe(
    ...
    cond([
        [
            type => equals(type, 'trigger') || equals(type, 'end'),
            () => '(({name}))'
        ],
        [
            equals('filter'),
        ...
        [
            type => equals(type, 'elementrequest') || equals(type, 'request'),
            () => '>{name}]'
        ],
        [
            equals('script'),
...
const getShapeStyle = pipe(
    ...
    cond([
        [
            equals('GENERATED'),
            () => `style {name} stroke-width:3px,stroke-dasharray: 5, 5;`
        ],
        [
            equals('WORMHOLE'),
            () => `style {name} fill:#dda7f9,stroke-width:3px,stroke-dasharray: 5, 5;`
        ],
        [
            equals('GET'),
            () => `style {name} stroke:#0f6ab4;`
        ],
        [
            equals('POST'),
            () => `style {name} stroke:#10a54a;`
        ],
        [
            equals('PATCH'),
            () => `style {name} stroke:#D38042;`
        ],
        [
            equals('PUT'),
            () => `style {name} stroke:#C5862B;`
        ],
        [
            equals('DELETE'),
            () => `style {name} stroke:#a41e22;`
        ]
    ])
)
...
const generateTipText = cond([
    ...
    [
        propEq('type', 'elementRequest'),
        s => ({ 'label': 'Request', 'text': objectToPrintOut(s.properties) })
    ],
    [
        propEq('type', 'loop'),
        s => ({ 'label': 'Loop', 'text': objectToPrintOut(s.properties) })
    ],
    [
        T,
        s =>  ({ 'label': `Other (${s.type})`, 'text': objectToPrintOut(s.properties) })
    ]
])

const encodeURLData = (stepJson, tipText, tipLabel) => {
    const toEncode = `<b>${tipLabel}:</b><br><br><pre>${tipText}</pre><br><b>RAW JSON:</b><br><br><pre>${jsTipText(JSON.stringify(stepJson, null, 2))}</pre>`
    const toEncode2 = `<html><head><title>${stepJson.name}</title></head><body>${toEncode}</body></html>`
    return `javascript:document.open();document.write(window.atob('${Buffer.from(toEncode2).toString('base64')}'));document.close();`
}
...
join('', map(step => `            ${stepNameMapping[step.name]}${step.shape.replace('{name}', step.label)}\n`)(steps))

const generateMermaidLinks = (steps, stepNameMapping) => 
join('', 
    chain(s => 
        map(l => `            ${stepNameMapping[s.name]}${l.line.replace('{link_label}', l.text)}${stepNameMapping[l.dest]}\n`)(s.links)
    )(steps))
...
join('', pipe(
    ...
    map(s => `            click ${stepNameMapping[s.name]} "${s.click_url}" "${s.tip_text.label}:<br><pre>${s.tip_text.text}</pre>"\n`)
)(steps))
...
const buildTable = (headers, values) => {
    ...
    const rows = join('', 
    map(value => rowTemplate
        .replace('{values}', 
        join('', 
        map(header => `<td>${value[header]}</td>`)(headers)
    )))(values))
...
module.exports = async (formula, formulaDirName) => {
    ...
    let maxCodeBlock = reduce(max, 0, pipe(
